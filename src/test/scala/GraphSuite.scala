...
class GraphSuite extends ScalaCheckSuite:
  ...
  // Property 1: Cyclic graphs should always return false
  property("cyclic graphs are never linear extensions") {
  ...
  // Property 2: Empty graph should accept empty list
  test("empty graph accepts empty list") {
    val emptyGraph = Graph.empty[Int, DiEdge[Int]]
    assert(isLinearExtension(emptyGraph)(List.empty).isLeft)
  }

  // Property 3: Single node graph
  ...
  // Property 4: Wrong node set should return false
  ...
  // Property 5: Valid topological orderings should return true
  ...
  // Property 6: Reversed valid ordering should often be invalid (unless trivial cases)
  ...
  // Property 7: Removing a node from valid ordering makes it invalid
  ...
  // Property 8: Duplicating a node makes it invalid
  ...
  // Property 9: Chain graphs have unique topological ordering
  property("chain graphs have unique valid ordering") {
    forAll(Gen.choose(2, 6)) { n =>
      ...
      assert(isLinearExtension(chainGraph)(nodes.reverse).isLeft, s"Chain should reject reverse ordering: ${nodes.reverse}")
    }
  }

  // Property 10: Star graphs (one root pointing to all others) accept multiple orderings
